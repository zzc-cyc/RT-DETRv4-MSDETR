...
class DINO(nn.Module):
    ...
    def __init__(
        ...
        self.transformer.decoder.class_embed_o2m = copy.deepcopy(self.class_embed[0])
        self.transformer.decoder.bbox_embed_o2m = copy.deepcopy(self.bbox_embed[0])
        ...
        self.transformer.decoder.class_embed_iou = copy.deepcopy(self.class_embed)
    ...
    def forward(self, batched_inputs):
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            outputs_iou = self.transformer.decoder.class_embed_iou[lvl](inter_states[lvl])
            ...
            if self.training == False:
                ...
                outputs_iou = outputs_iou.sigmoid()
                if len(outputs_ious) > 0:
                    outputs_iou = (outputs_iou ** 2.001 + outputs_ious[-1].sigmoid() ** 1.853) / 2.
                    outputs_iou = torch.pow(outputs_iou, 1./2)
                outputs_iou = inverse_sigmoid(outputs_iou)
        ...
        # tensor shape: [num_decoder_layers, bs, num_query, 1]
        
        # Calculate output coordinates and classes.
        outputs_classes_o2m = []
        outputs_coords_o2m = []
        for lvl in range(o2m_outputs["inter_states"].shape[0]):
            if lvl == 0:
                reference = o2m_outputs["init_reference"]
            else:
                reference = o2m_outputs["inter_references"][lvl - 1]
            reference = inverse_sigmoid(reference)
            outputs_class_o2m = self.class_embed[lvl](o2m_outputs["inter_states"][lvl])
            tmp = self.bbox_embed[lvl](o2m_outputs["inter_states"][lvl])
            if reference.shape[-1] == 4:
                tmp += reference
            else:
                assert reference.shape[-1] == 2
        ...
        outputs_classes_sep = []
        outputs_coords_sep = []
        for lvl in range(sep_outputs["inter_states"].shape[0]):
            if lvl == 0:
                reference = sep_outputs["init_reference"]
            else:
                reference = sep_outputs["inter_references"][lvl - 1]
            reference = inverse_sigmoid(reference)
            outputs_class_sep = self.class_embed[lvl](sep_outputs["inter_states"][lvl])
            tmp = self.bbox_embed[lvl](sep_outputs["inter_states"][lvl])
            if reference.shape[-1] == 4:
                tmp += reference
            else:
                assert reference.shape[-1] == 2
                tmp[..., :2] += reference
            outputs_coord_sep = tmp.sigmoid()
            outputs_classes_sep.append(outputs_class_sep)
            outputs_coords_sep.append(outputs_coord_sep)
        ...
        if dn_meta is not None:
            outputs_class, outputs_coord, outputs_iou = self.dn_post_process(
                outputs_class, outputs_coord, outputs_iou, dn_meta
            )

        # prepare for loss computation
        output = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1], "pred_ious": outputs_iou[-1]}
        if self.aux_loss:
            output["aux_outputs"] = [
        ...
        interm_iou = self.transformer.decoder.class_embed_iou[-1](enc_state)
        output["enc_outputs"] = {"pred_logits": interm_class, "pred_boxes": interm_coord, "pred_ious": interm_iou}
        
        interm_coord_o2m = o2m_outputs["enc_reference"]
        interm_class_o2m = self.transformer.decoder.class_embed_o2m(o2m_outputs["enc_state"])
        output["o2m_enc"] = {"pred_logits": interm_class_o2m, "pred_boxes": interm_coord_o2m}
        ...
        if self.training:
            ...
            box_cls = output["pred_logits"]
            box_pred = output["pred_boxes"]
            box_iou = output["pred_ious"]
            box_cls = inverse_sigmoid(box_cls.sigmoid() ** 0.5 * box_iou.sigmoid() ** 0.5)
            results = self.inference(box_cls, box_pred, images.image_sizes)
            processed_results = []
    ...
    def dn_post_process(self, outputs_class, outputs_coord, outputs_iou, dn_metas):
        if dn_metas and dn_metas["single_padding"] > 0:
            ...
            out = {"pred_logits": output_known_class[-1], "pred_boxes": output_known_coord[-1], "pred_ious": output_known_iou[-1]}
            if self.aux_loss:
                out["aux_outputs"] = [
                    {"pred_logits": a, "pred_boxes": b, "pred_ious": c}
                    for a, b, c in zip(output_known_class, output_known_coord, output_known_iou)
                ]
