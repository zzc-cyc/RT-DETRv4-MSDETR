...
from detrex.layers.box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou, box_xyxy_to_cxcywh
...
class Stage2Assigner(nn.Module):
    def __init__(self, num_queries, max_k=6, allow_low_quality_matches=True):
        super().__init__()
        self.positive_fraction = 0.25
        self.bg_label = 400  # number > 91 to filter out later
        self.batch_size_per_image = num_queries
        self.proposal_matcher = Matcher(thresholds=[0.4], labels=[0, 1], allow_low_quality_matches=allow_low_quality_matches)
        self.k = max_k
    ...
    def forward(self, outputs, targets, return_cost_matrix=False):
        ...
        for b in range(bs):
            ...
            matched_idxs, matched_labels = self.proposal_matcher(cost_matrix)  # proposal_id -> highest_iou_gt_id, proposal_id -> [1 if iou > 0.6, 0 ow]
