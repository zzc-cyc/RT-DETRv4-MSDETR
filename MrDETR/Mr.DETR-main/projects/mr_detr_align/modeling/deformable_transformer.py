    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.as_two_stage:
            ...
            # dn
            dn_query_embeds = kwargs['dn_query_embeds']
            if dn_query_embeds[1] is not None:
                reference_points = torch.cat([dn_query_embeds[1], reference_points], 1)
            ...
            o2m_reference_points = o2m_topk_coords_unact

            # dn
            if dn_query_embeds[1] is not None:
                o2m_reference_points = torch.cat([dn_query_embeds[1], o2m_reference_points], 1)

            o2m_init_reference_out = o2m_reference_points.sigmoid()
            o2m_pos_trans_out = self.pos_trans_norm(
                self.pos_trans(self.get_proposal_pos_embed(o2m_reference_points))
            )
            if not self.mixed_selection:
            ...
            o2m_reference_points = o2m_reference_points.sigmoid()
        ...
        if dn_query_embeds[0] is not None:
            o2m_query = torch.cat([dn_query_embeds[0], o2m_query], 1)
            query = torch.cat([dn_query_embeds[0], query], 1)
