...
class BaseTransformerLayer_Decoder(BaseTransformerLayer):
    # TODO: add more tutorials about BaseTransformerLayer
    ...
    def __init__(
        ...
        super(BaseTransformerLayer_Decoder, self).__init__(attn, ffn, norm, operation_order)
        # self.Linear_self = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        # self.Linear_cross = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        # self.Linear_self = nn.Sequential(
        #     nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8),
        #     nn.ReLU(),
        #     nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8),
        # )
        # self.Linear_cross = nn.Sequential(
        #     nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8),
        #     nn.ReLU(),
        #     nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8),
        # )
        self.Linear_self_before = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        self.Linear_self_after = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        self.Linear_cross_before = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        self.Linear_cross_after = nn.Conv1d(256, 256, kernel_size=1, stride=1, groups=8)
        self.Linear_self = nn.Sequential(
            nn.Conv1d(300, 300, kernel_size=1, stride=1, groups=300),
            nn.GELU(),
            nn.Conv1d(300, 300, kernel_size=1, stride=1, groups=300),
        )
        self.Linear_cross = nn.Sequential(
        ...
        **kwargs,
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                ...
                if temp_value.shape[1] == 300:
                    alpha = self.Linear_self_before(temp_value.permute(0, 2, 1)) + self.Linear_self_after(query.permute(0, 2, 1))
                    alpha = self.Linear_self(alpha.permute(0, 2, 1)).sigmoid()
                    query = query * alpha + temp_value * (1 - alpha)
                else:
                    alpha = self.Linear_self_before(temp_value[:, :300, :].permute(0, 2, 1)) + self.Linear_self_after(query[:, :300, :].permute(0, 2, 1))
                    alpha0 = self.Linear_self(alpha.permute(0, 2, 1)).sigmoid()
                    alpha = self.Linear_self_before(temp_value[:, 300:, :].permute(0, 2, 1)) + self.Linear_self_after(query[:, 300:, :].permute(0, 2, 1))
                    alpha1 = self.Linear_self(alpha.permute(0, 2, 1)).sigmoid()
                    alpha = torch.cat([alpha0, alpha1], dim=1)
                    query = query * alpha + temp_value * (1. - alpha)
                ...
                if temp_value.shape[1] == 300:
                    alpha = self.Linear_cross_before(temp_value.permute(0, 2, 1)) + self.Linear_cross_after(query.permute(0, 2, 1))
                    alpha = self.Linear_cross(alpha.permute(0, 2, 1)).sigmoid()
                    query = query * alpha + temp_value * (1 - alpha)
                else:
                    alpha = self.Linear_cross_before(temp_value[:, :300, :].permute(0, 2, 1)) + self.Linear_cross_after(query[:, :300, :].permute(0, 2, 1))
                    alpha0 = self.Linear_cross(alpha.permute(0, 2, 1)).sigmoid()
                    alpha = self.Linear_cross_before(temp_value[:, 300:, :].permute(0, 2, 1)) + self.Linear_cross_after(query[:, 300:, :].permute(0, 2, 1))
                    alpha1 = self.Linear_cross(alpha.permute(0, 2, 1)).sigmoid()
                    alpha = torch.cat([alpha0, alpha1], dim=1)
                    query = query * alpha + temp_value * (1. - alpha)
...
class BaseTransformerLayer_Encoder(BaseTransformerLayer):
    # TODO: add more tutorials about BaseTransformerLayer
    ...
    def __init__(
        ...
        super(BaseTransformerLayer_Encoder, self).__init__(attn, ffn, norm, operation_order)

        self.Linear_self_before = nn.Conv1d(256, 512, kernel_size=1, stride=1, groups=8)
        self.Linear_self_after = nn.Conv1d(256, 512, kernel_size=1, stride=1, groups=8)
        self.Linear_self = nn.Sequential(
            nn.Conv1d(512, 512, kernel_size=1, stride=1, groups=8),
            nn.GELU(),
            nn.Conv1d(512, 256, kernel_size=1, stride=1, groups=8),
        )
        ...
        **kwargs,
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                ...
                # alpha = self.Linear_self(query.permute(0, 2, 1)).permute(0, 2, 1).sigmoid()    
                # query = query * alpha + temp_value * (1 - alpha)
