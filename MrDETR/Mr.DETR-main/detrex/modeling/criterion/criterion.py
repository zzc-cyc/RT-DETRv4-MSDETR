...
class SetCriterion(nn.Module):
    ...
    def loss_class_vfl(self, outputs, targets, indices, num_boxes):
        assert "pred_logits" in outputs
        src_logits = outputs["pred_logits"]
        ...
        weight = 0.25 * pred_score.pow(2) * (1 - target) + target_score
        
        
        loss = F.binary_cross_entropy_with_logits(src_logits, target_score.detach(), weight=weight.detach(), reduction='none')
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes
        losses = {"loss_class": loss}
        return losses

    def loss_class_iabce(self, outputs, targets, indices, num_boxes, **kwargs):
        assert "pred_logits" in outputs
        src_logits = outputs["pred_logits"]
        ...
        ious, _ = box_iou(box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes))
        ious = torch.diag(ious).detach()
        # target = F.one_hot(target_classes, num_classes=80 + 1)[..., :-1]
        ...
        if 'userank' in kwargs and kwargs['userank']:
            rank = get_local_rank_o2m(t, indices)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "class_vfl": self.loss_class_vfl,
            "class_iabce": self.loss_class_iabce
        }
